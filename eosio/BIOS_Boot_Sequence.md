...
## eos构建系统启动顺序(BIOS Boot Sequence) - MAC
...
export EOSIO_SOURCE=/Users/tokenfun/taurus/eosio;
mkdir -p ${MBP_DIR}; cd ${MBP_DIR};
mkdir -p ${MBP_DIR}/logs/
...
###### B. 启动keosd并创建钱包 PW5HpWeJ1tULcPD6kb6sysgbCpZ92Zs5wcv52TAFGkb1PaveRFxHf

######## mbp_eosio ./start_keosd.sh
nohup keosd --unlock-timeout 99999999 --http-server-address 127.0.0.1:6666 --wallet-dir ${MBP_DIR}/wallet/ 2>&1 >> ${MBP_DIR}/logs/wallet.stderr &

######## mbp_eosio netstat -an | grep 6666
lsof -i tcp:6666 
COMMAND   PID     USER   FD   TYPE             DEVICE SIZE/OFF NODE NAME
keosd   38310 tokenfun   12u  IPv4 0x9a0d59dd88317bef      0t0  TCP localhost:6666 (LISTEN)
...
"PW5HpWeJ1tULcPD6kb6sysgbCpZ92Zs5wcv52TAFGkb1PaveRFxHf"

bash-3.2$ cat password-wallet.txt | xargs
...
bash-3.2$ cleos ${MBP_URL} wallet list keys
...
nohup nodeos --max-irreversible-block-age -1 --contracts-console --genesis-json ${MBP_DIR}/genesis.json --blocks-dir ${MBP_DIR}/nodes/bp-eosio/blocks --config-dir ${MBP_DIR}/nodes/bp-eosio   --data-dir ${MBP_DIR}/nodes/bp-eosio  --chain-state-db-size-mb 1024 --http-server-address 127.0.0.1:8000 --p2p-listen-endpoint 127.0.0.1:9000  --max-clients 25   --p2p-max-nodes-per-host 25  --enable-stale-production --producer-name eosio   --private-key '["EOS5kxVvnEMKnnW69Cxiseqn9BCu6zweKGkoLGDrX1gxtw3VVasWY","5J4PQwZiN6fjTMoZXCgnsqMNw1cvuQ8uDnpJme2JQjs4QXuxExn"]' --plugin eosio::http_plugin --plugin eosio::chain_api_plugin --plugin eosio::producer_plugin --plugin eosio::history_plugin --plugin eosio::history_api_plugin 2>&1 >> ${MBP_DIR}/logs/nodes_bp-eosio.stderr & 

###### --p2p-peer-address localhost:9000

bash-3.2$ ps -ef|grep bp-eosio
bash-3.2$ netstat -an |grep 9000
...
cleos ${MBP_URL} wallet unlock --password PW5HpWeJ1tULcPD6kb6sysgbCpZ92Zs5wcv52TAFGkb1PaveRFxHf
...
`````
cleos ${MBP_URL} set contract eosio.token ${EOSIO_SOURCE}/eos/build/contracts/eosio.token
cleos ${MBP_URL} set contract eosio.msig  ${EOSIO_SOURCE}/eos/build/contracts/eosio.msig
...
`````
cleos ${MBP_URL} set contract eosio ${EOSIO_SOURCE}/eos/build/contracts/eosio.system

问题2: Error 3080006: Transaction took too long
解决2: 重拾一次好了，是不是交易时间有限制呢？

问题3: Error 3050003: eosio_assert_message assertion failure
...
rm -rf ${MBP_DIR}/nodes/bp-producer21; mkdir -p ${MBP_DIR}/nodes/bp-producer21
nohup nodeos --max-irreversible-block-age -1 --contracts-console --genesis-json ${MBP_DIR}/genesis.json --blocks-dir ${MBP_DIR}/nodes/bp-producer21/blocks --config-dir ${MBP_DIR}/nodes/bp-producer21   --data-dir ${MBP_DIR}/nodes/bp-producer21  --chain-state-db-size-mb 1024 --http-server-address 127.0.0.1:8001 --p2p-listen-endpoint 127.0.0.1:9001  --max-clients 25   --p2p-max-nodes-per-host 25  --enable-stale-production --producer-name producer21   --private-key '["EOS7ZtdL3zpPvAaGjVkytno7LBPzNK8W5p7XURvRbaG1uC6d7Q4Lz","5Jm3kYjqMycqThnhmsXmxtTkmXkk7sC8T1NZFBedyCx1wiksoKW"]' --plugin eosio::http_plugin --plugin eosio::chain_api_plugin --plugin eosio::producer_plugin --plugin eosio::history_api_plugin --p2p-peer-address localhost:9000 2>&1 > ${MBP_DIR}/logs/nodes_bp-producer21.stderr &

rm -rf ${MBP_DIR}/nodes/bp-producer22; mkdir -p ${MBP_DIR}/nodes/bp-producer22
nohup nodeos --max-irreversible-block-age -1 --contracts-console --genesis-json ${MBP_DIR}/genesis.json --blocks-dir ${MBP_DIR}/nodes/bp-producer22/blocks --config-dir ${MBP_DIR}/nodes/bp-producer22   --data-dir ${MBP_DIR}/nodes/bp-producer22  --chain-state-db-size-mb 1024 --http-server-address 127.0.0.1:8002 --p2p-listen-endpoint 127.0.0.1:9002  --max-clients 25   --p2p-max-nodes-per-host 25  --enable-stale-production --producer-name producer22   --private-key '["EOS6PE79KUsKKGfbvK3Hv4NwREmG2JXaFWRjKJGx6KqHMLp5PUb55","5JStKAD4QtsJvYszHtJYHKncbX663kMEFyjktqAiQjNVn6hSzSy"]' --plugin eosio::http_plugin --plugin eosio::chain_api_plugin --plugin eosio::producer_plugin --plugin eosio::history_api_plugin --p2p-peer-address localhost:9000 --p2p-peer-address localhost:9001 2>&1 > ${MBP_DIR}/logs/nodes_bp-producer22.stderr &

rm -rf ${MBP_DIR}/nodes/bp-producer23; mkdir -p ${MBP_DIR}/nodes/bp-producer23
nohup nodeos --max-irreversible-block-age -1 --contracts-console --genesis-json ${MBP_DIR}/genesis.json --blocks-dir ${MBP_DIR}/nodes/bp-producer23/blocks --config-dir ${MBP_DIR}/nodes/bp-producer23   --data-dir ${MBP_DIR}/nodes/bp-producer23  --chain-state-db-size-mb 1024 --http-server-address 127.0.0.1:8003 --p2p-listen-endpoint 127.0.0.1:9003  --max-clients 25   --p2p-max-nodes-per-host 25  --enable-stale-production --producer-name producer23   --private-key '["EOS8WXoYfRF6kN3S59hp23VsETYF3tJkWNp8uxq866cWfqu8T2VLt","5Jxx3RTo5feTt9d779ZGKtB5CiaUp8EDx1aqcxLy2Ete6wFMHNy"]' --plugin eosio::http_plugin --plugin eosio::chain_api_plugin --plugin eosio::producer_plugin --plugin eosio::history_api_plugin --p2p-peer-address localhost:9000 --p2p-peer-address localhost:9001 --p2p-peer-address localhost:9002 2>&1 > ${MBP_DIR}/logs/nodes_bp-producer23.stderr &


rm -rf ${MBP_DIR}/nodes/bp-producer24; mkdir -p ${MBP_DIR}/nodes/bp-producer24
nohup nodeos --max-irreversible-block-age -1 --contracts-console --genesis-json ${MBP_DIR}/genesis.json --blocks-dir ${MBP_DIR}/nodes/bp-producer24/blocks --config-dir ${MBP_DIR}/nodes/bp-producer24   --data-dir ${MBP_DIR}/nodes/bp-producer24  --chain-state-db-size-mb 1024 --http-server-address 127.0.0.1:8004 --p2p-listen-endpoint 127.0.0.1:9004  --max-clients 25   --p2p-max-nodes-per-host 25  --enable-stale-production --producer-name producer24   --private-key '["EOS6wHVUDA1ouZuF3fuJAFP8GenXMM3cEFLivRj8vFjdhGnY3uhhk","5J1kryK5qqh9ePxfpgsHSYJsvUbHtRB4k8ES95YmTgrx3DugWH2"]' --plugin eosio::http_plugin --plugin eosio::chain_api_plugin --plugin eosio::producer_plugin --plugin eosio::history_api_plugin --p2p-peer-address localhost:9000 --p2p-peer-address localhost:9001 --p2p-peer-address localhost:9002 --p2p-peer-address localhost:9003 2>&1 > ${MBP_DIR}/logs/nodes_bp-producer24.stderr &

rm -rf ${MBP_DIR}/nodes/bp-producer25; mkdir -p ${MBP_DIR}/nodes/bp-producer25
nohup nodeos --max-irreversible-block-age -1 --contracts-console --genesis-json ${MBP_DIR}/genesis.json --blocks-dir ${MBP_DIR}/nodes/bp-producer25/blocks --config-dir ${MBP_DIR}/nodes/bp-producer25   --data-dir ${MBP_DIR}/nodes/bp-producer25  --chain-state-db-size-mb 1024 --http-server-address 127.0.0.1:8005 --p2p-listen-endpoint 127.0.0.1:9005  --max-clients 25   --p2p-max-nodes-per-host 25  --enable-stale-production --producer-name producer25   --private-key '["EOS65erFADQsFxiCUQmLyrWhVTinGowXmcgRhTDf6J6tKfGzqrBCk","5JBqqRMvQcgfmZ9XfccX5tAaFkmTJV9E9fsCcyC5cq31CpYvrW2"]' --plugin eosio::http_plugin --plugin eosio::chain_api_plugin --plugin eosio::producer_plugin --plugin eosio::history_api_plugin --p2p-peer-address localhost:9000 --p2p-peer-address localhost:9001 --p2p-peer-address localhost:9002 --p2p-peer-address localhost:9003 --p2p-peer-address localhost:9004 2>&1 > ${MBP_DIR}/logs/nodes_bp-producer25.stderr &
...
cleos ${MBP_URL} push action eosio buyrambytes '["producer21", "user21", 200000]' -p producer21

cleos ${MBP_URL} multisig propose fast.unstake '[{"actor": "producer21", "permission": "active"}, {"actor": "producer22", "permission": "active"}, {"actor": "producer23", "permission": "active"}, {"actor": "producer24", "permission": "active"}, {"actor": "producer25", "permission": "active"}]'  '[{"actor": "eosio", "permission": "active"}]' eosio setcode '{"account": "eosio", "vmtype": 0, "vmversion": 0, "code": "081a13526444a64c7e4a3098ac94ea14a743388a1b3d0bc6ac7068fbd1f9938c"}'  -p user21

cleos ${MBP_URL} multisig approve user21 fast.unstake '{"actor": "producer21", "permission": "active"}' -p producer21
cleos ${MBP_URL} multisig approve user21 fast.unstake '{"actor": "producer22", "permission": "active"}' -p producer22
cleos ${MBP_URL} multisig approve user21 fast.unstake '{"actor": "producer23", "permission": "active"}' -p producer23
cleos ${MBP_URL} multisig approve user21 fast.unstake '{"actor": "producer24", "permission": "active"}' -p producer24
cleos ${MBP_URL} multisig approve user21 fast.unstake '{"actor": "producer25", "permission": "active"}' -p producer25

cleos ${MBP_URL} multisig exec user21 fast.unstake -p user21
cleos ${MBP_URL} multisig cancel user21 fast.unstake -p user21
cleos ${MBP_URL} multisig review user21 fast.unstake


cleos ${MBP_URL} get code eosio
code hash: 081a13526444a64c7e4a3098ac94ea14a743388a1b3d0bc6ac7068fbd1f9938c
